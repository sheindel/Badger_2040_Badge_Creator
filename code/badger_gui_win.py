...
@app.command()
def main(
    conference: str = "BSides Fort Wayne 2024",
    formimage: str = "images/BadgeLogo.jpg",
    localpath: str = "/home/dprows/Documents/Badger_2040/",
):
    # Set global variables
    badge_logo = "/badges/BadgeLogo.jpg"
    badge_creator_logo = os.path.join(localpath, "images/BSidesLogo.png")
    badge_file = os.path.join(localpath, "code/badge.txt")
    badge_image = os.path.join(localpath, formimage)
    ...
    class BadgeForm:
        def __init__(self, master):
            ...
            self.label_event = tk.Label(master, text=conference, font=("Ariel", 25))
            self.label_event.pack()

            # Add form Title Label
            self.label_title = tk.Label(
                master, text="Badge Creator", font=("Ariel", 25)
            )
            self.label_title.pack()

            # Load and resize the image
            self.image = ImageTk.PhotoImage(
                Image.open(badge_creator_logo).resize((446, 278))
            )
            ...
            # Add Name Label
            self.label_firstname = tk.Label(
                self.label_container, text="First Name:", font=("Ariel, 18")
            )
            self.label_firstname.grid(row=0, sticky="e")

            # Add Name Label
            self.label_lastname = tk.Label(
                self.label_container, text="Last Name:", font=("Ariel, 18")
            )
            self.label_lastname.grid(row=1, sticky="e")

            # Add Company Label
            self.label_company = tk.Label(
                self.label_container, text="Company:", font=("Ariel, 18")
            )
            self.label_company.grid(row=2, sticky="e")

            # Add Status Label
            self.label_status = tk.Label(
                self.label_container, text="Status:", font=("Ariel, 18")
            )
            ...
            self.entry_firstname = tk.Entry(self.entry_container, bd=5)
            self.entry_firstname.grid(row=0, column=1)

            # Add Entry for Full Name
            self.entry_lastname = tk.Entry(self.entry_container, bd=5)
            ...
            self.status_dropdown = tk.OptionMenu(
                self.entry_container, self.status_var, *self.options
            )
            self.status_dropdown.config(font=("Ariel", 16))
            self.status_dropdown.grid(row=3, column=1, sticky="ew")
